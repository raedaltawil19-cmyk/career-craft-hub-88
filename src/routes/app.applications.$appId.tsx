...
function ApplicationDetail() {
  ...
  if (!app) {
    return (
      <EmptyState
        title={t("applications.appNotFoundTitle")}
        description={t("applications.appNotFoundDescription")}
        action={
          <Link
            to="/app/applications"
  ...
  return (
    <div className="space-y-5">
      ...
      >
        <ArrowLeft className="size-4 rtl:rotate-180" /> {t("applications.backLink")}
      </Link>
      ...
      <header className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm text-muted-foreground">{t("applications.appliedOn", { date: app.appliedDate })}</p>
        </div>
        <StatusPill status={app.status} />
      </header>

      <Panel>
        <h2 className="eyebrow">{t("applications.statusHeading")}</h2>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Panel className="lg:col-span-2">
          <h2 className="text-lg">{t("applications.timelineHeading")}</h2>
        ...
        <Panel>
          <h2 className="text-lg">{t("applications.detailsHeading")}</h2>
          <dl className="mt-3 space-y-3 text-sm">
            ...
            {app.jobId ? (
              <div>
                <dt className="eyebrow">{t("applications.linkedJobLabel")}</dt>
                <dd className="mt-1">
                  <Link
                    to="/app/jobs/$jobId"
                  ...
                  >
                    {t("applications.viewJobAnalysis")}
                  </Link>
                </dd>
              </div>
            ) : null}
            {app.link ? (
              <div>
                <dt className="eyebrow">{t("applications.postingLabel")}</dt>
                <dd className="mt-1">
                  <a
                    href={app.link}
                    target="_blank"
                  ...
                  >
                    <ExternalLink className="size-3.5 shrink-0" /> {t("applications.openLink")}
                  </a>
                </dd>
              </div>
            ) : null}
            {app.nextAction ? (
              <div>
                <dt className="eyebrow">{t("applications.nextStepLabel")}</dt>
                <dd className="mt-1">
                  {app.nextAction}
      ...
      <Panel>
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3">
          <h2 className="min-w-0 text-lg">{t("applications.notesHeading")}</h2>
          <button
            onClick={() => {
          ...
          >
            {savedNote ? t("applications.savedNotes") : t("applications.saveNotes")}
          </button>
        </div>
        <textarea
          value={notes}
      ...
      <Panel className="border-primary/25 bg-primary-soft/40">
        <div className="grid grid-cols-[auto_minmax(0,1fr)] gap-3">
          ...
          <div className="min-w-0">
            <h2 className="text-base font-semibold">{t("applications.assistantHeading")}</h2>
            <p className="mt-1 text-sm text-muted-foreground">{t("applications.assistantDescription")}</p>
            <div className="mt-3 flex flex-wrap gap-2">
              {[
                t("applications.promptPrepareInterview"),
                t("applications.promptDraftFollowup"),
                t("applications.promptGaps"),
              ].map((a) => (
                <button
                  key={a}
